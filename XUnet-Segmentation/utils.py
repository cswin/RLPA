...
def mae_dice_loss(y_true, y_pred):
    return mean_absolute_error(y_true, y_pred) + dice_loss(y_true, y_pred)
...
def WMAE_loss(y_true, y_pred):
    ...
    if y_pred!=None:
        ...
        xrow=y_true.shape[1]
        yrow=y_true.shape[1]
        y_true=np.reshape(y_true, (y_true.shape[1], y_true.shape[2]))
        y_pred=np.reshape(y_pred, (y_pred.shape[1], y_pred.shape[2]))
        result=np.zeros((xrow,yrow))
        for i in range(xrow):
            for j in range(yrow):
                if y_true[i,j]==255:
                   result[i,j]=weight_B*abs(y_true[i,j]-y_pred[i,j]) 
                else:
                   result[i,j]=weight_DC*abs(y_true[i,j]-y_pred[i,j]) 
     
        return K.mean(K.sum(result))/(weight_B+weight_DC)
...
def sorensen_dice_loss(y_true, y_pred, weights=[0,1]):
...
def jaccard_loss(y_true, y_pred, weights=[0,1]):
...
def test_model(myxunet):
    ...
    data_path="../data/train/trainImage_save_path_800/"
    save_path="../results/0142/ROI1024T700V_validation_v1_512/"
    out_rows=128 
    out_cols=128
#    original_rows=2056
#    original_cols=2124
...
#        img = load_img(data_path + "/" + midname,grayscale = False,target_size=(out_rows,out_cols))
...
#        img[:,:,2] -= 29.6568
        ...
        i += 1
    print('loading done')
    np.save(save_path + '/imgs_test_cropped.npy', imgs_test)
    ...
    #load test data
    imgs_test_data = np.load(save_path+"/imgs_test_cropped.npy")
    imgs_test_data = imgs_test_data.astype('float32')
#    imgs_test_data[:,:,:,0] -= 34.4804
#    imgs_test_data[:,:,:,1] -= 68.8604
#    imgs_test_data[:,:,:,2] -= 118.389
    imgs_test_data /= 255
    ...
    #save the results
    np.save(save_path+'/imgs_test_results_cropped.npy', imgs_mask_test)
    
    #get the results as images format
    print("array to image")
    results = np.load(save_path+'/imgs_test_results_cropped.npy')#load the saved result
...
#    		img[img < 0.5] = 1
    		...
    		print(midname)
    		save_images(os.path.join(save_path, '%s' % midname), img)
            
#    		img1 = Image.open(os.path.join(save_path, '%s.bmp' % midname ))
#       
#    		img1 = img1.resize((original_cols, original_rows), Image.BILINEAR) 
#    
#    		save_images(os.path.join(save_path, '%s.bmp' % midname ), img1)